...
export default function ProfilePage() {
  const { user, profile, bookmarks, recentlyViewed } = useApp();
    ...
    <div className="container py-8">
      <div className="glass-card rounded-2xl p-6 mb-8 flex flex-col sm:flex-row items-center gap-6">
        ...
        <div className="text-center sm:text-left">
          <h1 className="text-2xl font-display font-bold">{profile?.display_name || "User"}</h1>
      ...
      <section>
        ...
        {recentTools.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {recentTools.map((tool, i) => <ToolCard key={tool.id} tool={tool} index={i} />)}
          </div>
        )}
      </section>
    </div>
